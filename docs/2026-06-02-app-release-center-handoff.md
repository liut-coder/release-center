...
- 无数据库模式已从只读兜底升级为进程内可变 Demo store，覆盖 App、构建、发布、资源版本、设备、事件、审计和系统管理数据；本地浏览器 smoke 可用 demo 项目真实提交系统管理用户/角色/权限/数据字典/菜单创建和启停/显隐、App 创建和启停、构建创建、发布草稿创建、发布/撤回/回滚/下架/暂停/灰度/编辑说明、资源 ZIP 上传创建、资源发布/回滚/暂停/灰度/编辑说明，并在操作审计页按版本搜索到对应留痕。
- App 构建创建成功后，发布草稿表单会自动选中刚创建的成功构建并同步渠道，避免连续创建构建和发布时误选旧构建。
- App 发布页面移动端长版本号、长文件名、select 和操作按钮已补齐响应式约束，提交真实 demo 数据后仍通过页面横向溢出检查。
- 系统管理 role 响应已标准化空权限数组，避免 `permissions:null` 导致前端角色表格白屏；`system.admin` 可见自定义菜单路径，`release.admin` 仍只保留首页和发布中心菜单。
...
| 前端闭环 | `web/` 管理台已具备登录、首页、系统管理页面、用户/角色/权限/数据字典/菜单后端数据接入、菜单/RBAC 过滤结果驱动导航和 App 发版中心业务模块；本地 Playwright 已验证桌面/移动系统管理写入和启停/显隐、App 管理创建和启停、App 发版表单提交、发布/撤回/回滚/下架/暂停、资源发布/回滚/暂停、操作审计检索、运行时错误监听和无横向溢出 | 生产静态资源浏览器 smoke |
...
| 操作审计 | `InsertAudit` 覆盖 release/resource/webhook 操作，真实 smoke 覆盖 `resource.auto_pause`，浏览器 smoke 覆盖发布和资源操作审计检索 | 已验证 |
...
本地浏览器 smoke 结果：已安装 Playwright Chromium，`npm run smoke:browser` 通过 4 个用例，覆盖桌面和移动视口、`release.admin` / `system.admin` 登录、RBAC 菜单裁剪、系统管理页、用户创建和启停、角色创建和启停、权限创建和启停、数据字典创建和启停、菜单创建和显隐、App 发版中心概览 / 构建记录 / App 发布 / 资源增量 / 设备版本 / 升级统计 / 升级事件 / 操作审计、App 管理创建和启停、构建创建、发布草稿创建、发布/撤回/回滚/下架/暂停/灰度/编辑说明、合法资源 ZIP 上传创建、资源发布/回滚/暂停/灰度/编辑说明、按版本检索发布和资源操作审计、控制台错误、页面错误、Admin/API 5xx 监听和页面横向溢出检查。生产环境仍需对真实域名、真实 PostgreSQL 和生产 token 映射复跑。
...
web browser smoke                      `npm run smoke:browser` 通过，Chromium desktop/mobile 共 4 个用例，含 demo 系统管理新增/状态操作、App 管理创建/启停、App 发版表单提交、发布/撤回/回滚/下架/暂停、资源发布/回滚/暂停和审计检索
