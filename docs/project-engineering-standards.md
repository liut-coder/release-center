...
留档和文档更新是功能完成标准的一部分，不是收尾附属工作。功能已经合并但文档没有记录时，默认视为未完成交付。
...
- 如果一次改动跨越多个模块，必须在交接文档或实现文档中写清楚影响范围和回归重点。
- 如果因为时间、环境或外部依赖无法验证，必须把未验证原因、风险等级和后续验证入口写入文档。
- 文档中的状态描述必须使用明确结论，例如“已验证”“待生产验证”“仅本地 demo 验证”“待真机验证”，避免只写“基本完成”。
...
- 代码新增了接口，文档要同步记录路径、认证、关键参数和验证方式。
- 前端新增了页面、菜单或状态操作，文档要同步记录入口、权限和 smoke 覆盖情况。
- 数据库新增了表、字段、索引或 migration，文档要同步记录用途、回滚思路和生产验证要求。
- 权限、token、账号映射或菜单可见性发生变化，文档要同步记录角色边界，不记录明文密钥。
- 验证命令结果发生变化，交接文档要同步更新最近一次结果，不能沿用过期结论。
...
- `git status --short` 无遗漏文件，或明确说明哪些文件暂不提交及原因。
- `git log --oneline -5` 能看出最近几个功能点或里程碑的推进顺序。
- 交接文档中的最新 commit、已 push 状态、验证结果与仓库实际状态一致。
- 后续任务有明确优先级，不能只写笼统的“继续优化”。
- 若存在生产、真机、外部 CI 等未验证事项，必须写清楚所需环境和最小验证步骤。
