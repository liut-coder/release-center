...
- 支持 Webhook Route dry-run：用户输入事件和 ref 后，后台复用真实 route 匹配逻辑返回命中结果、build ref 和阻塞原因，但不创建构建任务，用于在真实 GitHub push/tag 前做本地验收。
...
多环境发布计划、部署记录、外部 worker API、审批门禁和路由层 RBAC 已进入当前分支；Cloudflare 真部署和真实生产回滚继续放在后续小分支，避免一次改动过大。
